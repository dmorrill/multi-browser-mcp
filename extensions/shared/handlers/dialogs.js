...
export class DialogHandler {
  ...
  async setupDialogOverrides(tabId, accept = true, promptText = '') {
    ...
    try {
      // Inject dialog override script
      await this.browserAdapter.executeScript(tabId, {
        code: this._generateDialogOverrideScript(dialogResponse)
      });
  ...
  /**
   * Generate the dialog override script
   * @private
   */
  _generateDialogOverrideScript(dialogResponse) {
    return `
  ...
  async getDialogEvents(tabId) {
    try {
      const results = await this.browserAdapter.executeScript(tabId, {
        code: 'window.__blueprintDialogEvents || []'
      });
